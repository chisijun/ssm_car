/*提取接口公共部分*/
var ApiConf = {
    server: "http://localhost:8080/car/",
    token: window.sessionStorage.getItem('token'),
};

function request(api, callback, info, type, token) {
    //加载动画
    layer.load(2);
    //参数
    var data = info;
...
function user_login(userName, password) {
    ...
    request("login/token", function(arr) {
        if (arr.status == false) {
    ...
    request("user", function(arr) {
        if (arr.status == false) {
            ...
            if (arr.data.list != '' && arr.data.roleCode == 'admin') {
                ...
                window.sessionStorage.setItem('roleId', arr.data.roleId);
...
function logout() {
    request("user/loginOut", function(arr) {
        if (arr.status) {
