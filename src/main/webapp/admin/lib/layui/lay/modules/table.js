...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function (exports) {
        ...
        if (options.url) { //Ajax请求
            ...
            $.ajax({
                ...
                , success: function (res) {
                    ...
                    // that.renderData(res, curr, res[response.countName]), sort();--修改
    ...
    Class.prototype.renderData = function (res, curr, count, sort) {
            ...
            , render = function () { //后续性能提升的重点
                ...
                layui.each(data, function (i1, item1) {
                    var tds = [], tds_fixed = [], tds_fixed_r = []
                        , numbers = i1 + options.limit * (curr - 1) + 1, rowevent="",rowstyle=""; //序号
                    ...
                    if (options.onRowClick != undefined && options.onRowClick != "") {
                        rowevent = 'row-event=true';
                    } 
                    if (options.rowStyle != undefined && options.rowStyle != "") {
                        rowstyle = options.rowStyle(i1, item1);
                    } 

                    trs.push('<tr ' + rowevent+' '+rowstyle+' data-index="' + i1 + '">' + tds.join('') + '</tr>');
                    trs_fixed.push('<tr  ' + rowevent + ' ' + rowstyle +' data-index="' + i1 + '">' + tds_fixed.join('') + '</tr>');
                    trs_fixed_r.push('<tr  ' + rowevent + ' ' + rowstyle +' data-index="' + i1 + '">' + tds_fixed_r.join('') + '</tr>');
        ...
        if (options.page) {
            options.page = $.extend({
                ...
                , count: count
                , limit: options.limit
                , limits: options.limits || [10, 20, 30, 40, 50, 100,150,200]
                , groups: 3
    ...
    Class.prototype.events = function () {
        ...
        th.on('click', function () {
            ...
            //that.sort(othis, type, null, true);--修改
            that.remoteSort(othis, type, null, true);
            ...
            layui.stope(e);
            if (index === 0) {
                // that.sort(field, 'asc', null, true);--修改
                that.remoteSort(field, 'asc', null, true);
            } else {
                // that.sort(field, 'desc', null, true);--修改
                that.remoteSort(field, 'desc', null, true);
            }
        });
        ...
        //行点击
        that.layBody.on('click', '*[row-event]', function () {
            var othis = $(this)
                , index = othis.eq(0).data('index')
                , tr = $(this)
                , ELEM_CLICK = 'layui-table-click'
                , data = table.cache[that.key][index];
            options.onRowClick(index, data,tr);
            tr.addClass(ELEM_CLICK).siblings('tr').removeClass(ELEM_CLICK);
        });
